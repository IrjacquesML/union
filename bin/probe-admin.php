...
$fail = 0;
if ($auth['code'] !== 200 || str_contains($auth['body'], 'name="password"')) {
    $fail++;
}

$front = [
    '/', '/pages/histoire', '/pages/vision', '/pages/mission', '/croyances',
    '/dirigeants', '/dirigeants/pr-jean-mukendi',
    '/departements', '/departements/jeunesse',
    '/associations', '/associations/eclaireurs-uco',
    '/mediatheque', '/mediatheque/message-bienvenue-union', '/galeries/assemblee-2025',
    '/actualites', '/actualites/bienvenue-nouveau-site-uco',
    '/evenements', '/evenements/camp-meeting-union-2026',
    '/contact',
];
foreach ($front as $path) {
    $r = http('GET', $base . $path, $cookieFile);
    $issue = failHint($r);
    if ($r['code'] !== 200) {
        $issue = $issue !== '' ? $issue : ('HTTP ' . $r['code']);
    }
    $mark = $issue === '' ? 'OK' : 'FAIL';
    if ($issue) {
        $fail++;
...
$pages = [
    ...
    '/admin/galleries', '/admin/galleries/create',
...
$crud = [
    ...
    [
        ...
        'post' => '/admin/settings',
        'data' => [
            'site_name' => 'Union du Congo Ouest — UCO',
            'site_tagline' => 'Adventistes du 7e Jour — Kinshasa',
...
$contactPage = http('GET', $base . '/contact', $cookieFile);
$contactToken = csrfFrom($contactPage['body']);
if (!$contactToken) {
    echo sprintf("%-4s %-36s %s\n", 'FAIL', 'contact form', 'no csrf');
    $fail++;
} else {
    $resp = http('POST', $base . '/contact', $cookieFile, [
        '_csrf' => $contactToken,
        'name' => 'Visiteur Probe',
        'email' => 'visiteur@uco.local',
        'phone' => '',
        'subject' => 'Test contact',
        'message' => 'Message de verification automatique.',
    ], false);
    $loc = '';
    if (preg_match('/^Location:\s*(.+)$/mi', $resp['headers'], $lm)) {
        $loc = trim($lm[1]);
    }
    $ok = $resp['code'] >= 300 && $resp['code'] < 400 && str_contains($loc, '/contact');
    if ($ok) {
        $followUrl = str_starts_with($loc, 'http') ? $loc : $base . $loc;
        $fr = http('GET', $followUrl, $cookieFile);
        $ok = str_contains($fr['body'], 'Votre message a bien été envoyé');
    }
    if (!$ok) {
        $fail++;
    }
    echo sprintf("%-4s %-36s %s\n", $ok ? 'OK' : 'FAIL', 'contact form', $ok ? '' : ('HTTP ' . $resp['code'] . ' loc=' . $loc));
}
