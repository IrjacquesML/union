...
<form method="post" action="<?= $event ? '/admin/events/' . (int) $event['id'] : '/admin/events' ?>" enctype="multipart/form-data" class="form admin-form">
    <?= csrf_field() ?>
    <label>Titre *</label>
    ...
    <?php if (!empty($event['cover_image'])): ?><img src="<?= e($event['cover_image']) ?>" alt="" class="thumb"><?php endif; ?>
    <div class="form-grid">
