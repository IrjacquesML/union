...
<section class="section">
    <div class="container">
        ...
        <?php else: ?>
            <p class="muted">Aucun dirigeant d’Union publié pour le moment.</p>
        <?php endif; ?>
    </div>
</section>
<?php
$otherCurrent = array_values(array_filter($current ?? [], static fn(array $row): bool => ($row['scope_type'] ?? '') !== 'union'));
if ($otherCurrent):
?>
<section class="section section-alt">
    <div class="container">
        <h2>Autres responsables en mandat</h2>
        <div class="people-grid">
            <?php foreach ($otherCurrent as $person): ?>
                <article class="person">
                    <div class="person-photo">
                        <?php if (!empty($person['photo'])): ?>
                            <img src="<?= e($person['photo']) ?>" alt="">
                    ...
                    <h3><a href="/dirigeants/<?= e($person['slug']) ?>"><?= e(trim(($person['title_prefix'] ?? '') . ' ' . $person['first_name'] . ' ' . $person['last_name'])) ?></a></h3>
                    <p><?= e($person['position_title']) ?></p>
                    <p class="muted"><?= e($person['scope_name'] ?? '') ?></p>
                </article>
            <?php endforeach; ?>
        </div>
    </div>
